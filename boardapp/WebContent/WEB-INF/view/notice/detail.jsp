...
<head>
    <title>코딩 전문가를 만들기 위한 온라인 강의 시스템 - 공지상세내용</title>
    <meta charset="UTF-8">
							...
							<tbody>
								<tr>
									...
									<td colspan="3"  class="text-align-left text-indent">
										<c:if test="${next_notice == null}">
											다음글이 없습니다
										</c:if>
										<c:if test="${next_notice != null}">
											<a class='text-blue text-strong' href='?id=${next_notice.id}'>${next_notice.title}</a>
										</c:if>
									</td>
								</tr>
								<tr>
									<th>이전글</th>
									<td colspan="3"  class="text-align-left text-indent">
										<c:if test="${prev_notice == null}">
											이전글이 없습니다
										</c:if>
										<c:if test="${prev_notice != null}">
											<a class='text-blue text-strong' href='?id=${prev_notice.id}'>${prev_notice.title}</a>
										</c:if>
									</td>
								</tr>
							</tbody>
